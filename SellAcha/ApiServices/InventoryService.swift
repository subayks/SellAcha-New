...
protocol InventoryServiceProtocol {
    func inventoryCall(finalURL: String, httpHeaders: [String: String], withParameters: String, completion: @escaping(_ status: Bool?, _ code: String?, _ response: AnyObject?, _ error: String?)-> Void)
    
}

class InventoryService: InventoryServiceProtocol {
    
    func inventoryCall(finalURL: String, httpHeaders: [String : String], withParameters: String, completion: @escaping (Bool?, String?, AnyObject?, String?) -> Void) {
        let headers = [
        ...
        NetworkAdapter.clientNetworkRequestCodable(withBaseURL: finalURL, withParameters: "", withHttpMethod: "POST", withContentType: "Application/json", withHeaders: headers, completionHandler: { (result: Data?, showPopUp: Bool?, error: String?, errorCode: String?)  -> Void in
            ...
            DispatchQueue.main.async {
                ...
                do {
                    ...
                    let values = try decoder.decode(BaseResponse<InventoryModel>.self, from: result!)
