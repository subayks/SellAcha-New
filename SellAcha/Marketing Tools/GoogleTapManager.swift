...
class GoogleTapManager: UIViewController {
    @IBOutlet weak var buttonSave: UIButton!
    @IBOutlet weak var overView: UIView!
    
    @IBOutlet weak var googleTagField: UITextField!
    @IBOutlet weak var selectionTableView: UITableView!
    @IBOutlet weak var buttonStatus: UIButton!
    @IBOutlet weak var statusFieldValue: UITextField!
    let vm = GoogleTapManagerVM()
    ...
    @IBAction func actionSave(_ sender: Any) {
        if self.googleTagField.text != "" {
            self.vm.postGoogleTapManager(type: "tag-manager", tagid: self.googleTagField.text ?? "", status: self.statusFieldValue.text ?? "")
        } else {
            let alert = UIAlertController(title: "Alert", message: "Please fill required field", preferredStyle: UIAlertController.Style.alert)
            alert.addAction(UIAlertAction(title: "Ok", style: UIAlertAction.Style.default, handler: nil))
            self.present(alert, animated: true, completion: nil)
        }
    }
...
extension GoogleTapManager: UITableViewDelegate, UITableViewDataSource {
